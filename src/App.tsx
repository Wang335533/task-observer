...
const checkStates: Record<string, string> = { checking: '正在检查', success: '检查完成', timeout: '检查超时', error: '检查异常', waiting: '等待首次检查' }
...
function TaskCard({ task, open }: { task: Task; open: (id: string) => void }) {
  ...
  return <a className="task-card" href={`#/task/${task.config.id}`} onClick={e => { e.preventDefault(); open(task.config.id) }} aria-label={`查看 ${task.config.name} 详情`}>
    ...
    <div className="freshness"><Clock3 size={13} /><span>{task.snapshot.statistics_at ? `${task.snapshot.statistics_kind || '源统计'} ${date(task.snapshot.statistics_at)}` : '暂无业务统计'}{task.snapshot.cached ? ' · 含缓存' : ''}</span></div><div className="check-cadence" title={`开始：${date(task.check_started_at)}；最近成功：${date(task.last_success_at)}；下次检查：${date(task.next_check_at)}`}><span>{task.checking ? '正在检查进度…' : task.last_checked_at ? `${checkStates[task.check_status || 'success']} ${date(task.last_checked_at)}` : '等待首次检查'}</span><br />每 5 分钟检查 · 单次限时 30 秒</div>
...
function Detail({ task, back, edit, names }: { task: Task; back: () => void; edit: () => void; names: Record<string, string> }) {
  ...
  return <>
    ...
    <div role="tabpanel">
      {tab === 'overview' && <>
        <div className="detail-metrics">{task.snapshot.metrics.map(m => <section className="panel detail-metric" key={m.key}><span>{m.label}</span><strong>{number(m.value)}<small>{m.unit}</small></strong><span className="muted">{task.snapshot.statistics_kind || '源统计'} {date(m.statistics_at === undefined ? task.snapshot.statistics_at : m.statistics_at)}{m.cached ? ' · 缓存' : ''}</span></section>)}</div>
        <div className="detail-columns"><section className="panel progress-panel"><div className="panel-title"><h3>业务进展</h3><span>来源：{task.config.adapter === 'grok' ? '只读进度模块' : task.config.adapter === 'process' ? '进程信息' : task.config.adapter === 'ssrn' ? '本地助手只读接口' : task.config.adapter === 'cnki' ? '只读期次汇总' : task.config.adapter === 'kokusho' ? '清单与分片' : '状态快照'}</span></div><div className="stage-focus"><div className="stage-mark"><Activity size={21} /></div><div><span className="eyebrow">当前阶段</span><h2>{task.snapshot.stage}</h2>{task.snapshot.current && <p className="mono">{task.snapshot.current}</p>}</div></div>
          ...
          <div className="queue-grid">{task.snapshot.queues.map(m => <div key={m.key}><span>{m.label}{m.cached ? ' · 缓存' : ''}</span><strong>{number(m.value)}</strong>{m.statistics_at !== undefined && <small className="muted">读取于 {date(m.statistics_at)}</small>}</div>)}</div>
        </section><section className="panel health-panel"><div className="panel-title"><h3>运行状况</h3><ShieldCheck size={18} /></div><StatusNotice task={task} />{task.view.issues.slice(1).map(i => <p className="issue-line" key={i.code}><TriangleAlert size={14} />{i.message}</p>)}<dl><div><dt>检查结果</dt><dd>{checkStates[task.check_status || "waiting"]}</dd></div><div><dt>检查开始</dt><dd>{date(task.check_started_at)}</dd></div><div><dt>检查结束</dt><dd>{date(task.last_checked_at)}</dd></div><div><dt>最近成功</dt><dd>{date(task.last_success_at)}</dd></div><div><dt>下次检查</dt><dd>{date(task.next_check_at)}</dd></div><div><dt>本次耗时</dt><dd>{number(task.read_duration)} 秒</dd></div><div><dt>{task.snapshot.statistics_kind || "源统计"}</dt><dd>{date(task.snapshot.statistics_at)}</dd></div><div><dt>任务进程</dt><dd>{task.resource.roots.length ? task.resource.roots.map(r => r.pid).join('、') : '当前未识别到'}</dd></div><div><dt>CPU</dt><dd>{number(task.resource.cpu_percent)}%</dd></div><div><dt>内存</dt><dd>{bytes(task.resource.memory_bytes)}</dd></div></dl><small className="muted">CPU 按整机总算力归一化；内存为关联进程工作集之和。</small></section></div>
  ...
  return <div className="app-shell">
    ...
    <main>
      ...
      {route === '/' && <>
        ...
        <div className="page-foot"><span><ShieldCheck size={13} />仅监控已指定的任务</span><span>资源检查：{date(data?.last_scan)}</span></div>
      </>}
