...
function TaskIcon({ adapter, size = 23 }: { adapter: string; size?: number }) { return adapter === 'grok' ? <Link2 size={size} /> : adapter === 'tieba' ? <MessageSquare size={size} /> : <Activity size={size} /> }
...
function StatusNotice({ task }: { task: Task }) {
  ...
  return issue ? <div className={`task-notice ${issue.level === 'error' ? 'error' : ''}`}><TriangleAlert size={15} /><span>{issue.message}</span></div> : <div className="task-notice quiet"><ShieldCheck size={15} /><span>{task.view.cooldown ? '正在等待冷却结束' : task.view.run_state === 'running' ? '任务正在运行' : '继续按原来的方式运行任务即可'}</span></div>
}
...
function TaskCard({ task, open }: { task: Task; open: (id: string) => void }) {
  ...
  return <a className="task-card" href={`#/task/${task.config.id}`} onClick={e => { e.preventDefault(); open(task.config.id) }} aria-label={`查看 ${task.config.name} 详情`}>
    ...
    <div className="freshness"><Clock3 size={13} /><span>{task.snapshot.statistics_at ? `统计 ${relative(task.snapshot.statistics_at)}更新` : '暂无业务统计'}{task.snapshot.cached ? ' · 缓存值' : ''}</span></div>
...
function Detail({ task, back, edit, names }: { task: Task; back: () => void; edit: () => void; names: Record<string, string> }) {
  ...
  useEffect(() => { void refresh(); const timer = setInterval(() => void refresh(), tab === 'logs' ? 10000 : 60000); return () => clearInterval(timer) }, [refresh, tab])
  return <>
    ...
    <div role="tabpanel">
      {tab === 'overview' && <>
        ...
        <div className="detail-columns"><section className="panel progress-panel"><div className="panel-title"><h3>业务进展</h3><span>来源：{task.config.adapter === 'grok' ? '只读进度模块' : task.config.adapter === 'process' ? '进程信息' : '状态快照'}</span></div><div className="stage-focus"><div className="stage-mark"><Activity size={21} /></div><div><span className="eyebrow">当前阶段</span><h2>{task.snapshot.stage}</h2>{task.snapshot.current && <p className="mono">{task.snapshot.current}</p>}</div></div>
        ...
        </section><section className="panel health-panel"><div className="panel-title"><h3>运行状况</h3><ShieldCheck size={18} /></div><StatusNotice task={task} />{task.view.issues.slice(1).map(i => <p className="issue-line" key={i.code}><TriangleAlert size={14} />{i.message}</p>)}<dl><div><dt>进度检查</dt><dd>{date(task.snapshot.updated_at)}</dd></div><div><dt>业务统计</dt><dd>{date(task.snapshot.statistics_at)}</dd></div><div><dt>任务进程</dt><dd>{task.resource.roots.length ? task.resource.roots.map(r => r.pid).join('、') : '当前未识别到'}</dd></div><div><dt>CPU</dt><dd>{number(task.resource.cpu_percent)}%</dd></div><div><dt>内存</dt><dd>{bytes(task.resource.memory_bytes)}</dd></div></dl><small className="muted">CPU 按整机总算力归一化；内存为关联进程工作集之和。</small></section></div>
...
export default function App() {
  ...
  useEffect(() => { active.current = true; void refresh(); const timer = setInterval(() => void refresh(), 5000); return () => { active.current = false; clearInterval(timer) } }, [refresh])
