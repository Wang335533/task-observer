...
- Grok：在短生命周期读取组件内按项目隔离加载原有只读进度模块，保留项目自身的配置和活动数据库定位逻辑；不启动外部 Python 或 PowerShell，不带 `--save`，不写 `__pycache__`。数据库单项查询限 0.35 秒，共享 3 秒预算，每 5 分钟检查一次；原统计时间与部分缓存标记保留。缓存使用观测台自己的最近有效结果，按数据源和运行隔离，不退回项目中陈旧的监控基线；保留字段原时间，源统计过期单独提醒。超时项保持未知或沿用明确标记的旧值，不把读取异常当成业务失败。
...
- 自动周期统一为 300 秒，各任务按自己的检查开始时间独立计时；首次启动立即检查，保存关注规则仅重查该任务。单任务不重叠，休眠恢复不补跑积压周期；完整读取限时 30 秒（含启动与输入输出），超时终止本应用的读取组件并保留旧统计。采集完成通过本地事件立即更新界面，不再等待下个周期。卡片同时展示统计时间和检查时间；检查发生并不保证源数据有变化。
...
React 界面通过 `collector_request(method, params)` 请求 Tauri；Rust 将限定方法转为带请求编号的 JSON 行发送至 Python 采集器。响应与请求对应，异常通知为独立消息。采集器后台线程采样资源；每个到期任务启动一个短生命周期的自有读取组件，使用打包采集器自身的 `--probe` 模式，完成后退出；慢任务不会占满共享线程池。关闭或失去父采集器后读取组件退出，不终止业务进程。前端使用持久化采集器世代和递增快照版本拒绝旧响应。SQLite 只保存仪表盘自己的信息。
